...
func newRefreshPolicyFactory(configFetcher configProvider, cache ConfigCache, logger Logger) *refreshPolicyFactory {
	return &refreshPolicyFactory{configFetcher: configFetcher, cache: cache, logger: logger}
}

func (factory *refreshPolicyFactory) visitAutoPoll(config autoPollConfig) refreshPolicy {
	return newAutoPollingPolicy(factory.configFetcher, factory.cache, factory.logger, config)
}

func (factory *refreshPolicyFactory) visitManualPoll(config manualPollConfig) refreshPolicy {
	return newManualPollingPolicy(factory.configFetcher, factory.cache, factory.logger)
}

func (factory *refreshPolicyFactory) visitLazyLoad(config lazyLoadConfig) refreshPolicy {
	return newLazyLoadingPolicy(factory.configFetcher, factory.cache, factory.logger, config)
}
